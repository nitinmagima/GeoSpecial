...
with tab1:
    ...
    # Embed iframe view of external dashboard
    st.subheader("Live Crisis Response Monitor")
    components.iframe("https://monitoring-crisis-response.lovable.app", height=600, scrolling=True)
...
with tab5:
    st.header("IFRC Real-Time Emergency Monitoring")
    components.iframe(
        "https://go.ifrc.org/countries/27/ongoing-activities/emergencies",
        height=600,
        scrolling=True
    )

with tab6:
    st.header("UNOSAT Bangladesh Emergency Events (2020)")

    if st.button("🗺️ Open UNOSAT Emergency Events (2020) in New Tab"):
        components.html(
            """
            <script>
                window.open("https://unosat.org/products/?date_from=2020-01-01&date_to=2020-12-31&region=Bangladesh", "_blank");
            </script>
            """,
            height=0,
        )
...
with tab7:
    st.header("PreventionWeb Publications on Disasters in Bangladesh")

    if st.button("📚 Open PreventionWeb Search in New Tab"):
        components.html(
            """
            <script>
                window.open("https://www.preventionweb.net/search#query=bangladesh", "_blank");
            </script>
            """,
            height=0,
        )
