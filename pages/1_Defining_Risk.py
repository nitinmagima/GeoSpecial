...
with hazard_tab:
    ...
    with historical_tab:
        ...
        populated_gdf = load_populated_places()
        if populated_gdf is None:
            st.warning("Could not load populated places data.")

        education_gdf = load_education_facilities()
        if education_gdf is None:
            st.warning("Could not load education facility data.")

        st.subheader("Select Map Layers to Display")

        show_boundary = st.checkbox("Show Admin Boundaries", value=True)
        show_cyclone = st.checkbox("Show Cyclone Track", value=True)
        show_populated = st.checkbox("Show Health Facilities", value=False)
        show_education = st.checkbox("Show Education Facilities", value=False)
        ...
        if boundary_gdf is not None and track_df is not None:
            ...
            if populated_gdf is not None:
                populated_layer = pdk.Layer(
                    "ScatterplotLayer",
                    data=populated_gdf,
                    get_position='geometry.coordinates',
                    get_color=[255, 165, 0, 180],  # Orange
                    get_radius=4000,
                    pickable=True,
                    tooltip=True
                )
            ...
            if education_gdf is not None:
                education_layer = pdk.Layer(
                    "ScatterplotLayer",
                    data=education_gdf,
                    get_position='geometry.coordinates',
                    get_color=[50, 205, 50, 180],  # Lime green
                    get_radius=3000,
                    pickable=True,
                    tooltip=True
                )
            else:
                education_layer = None
            ...
            if show_cyclone and track_df is not None:
                layers.append(track_layer)

            if show_populated and populated_gdf is not None:
                layers.append(populated_layer)

            if show_education and education_gdf is not None:
                layers.append(education_layer)
